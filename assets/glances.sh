...
if [[ $distrib_name == "ubuntu" || $distrib_name == "LinuxMint" || $distrib_name == "debian" || $distrib_name == "Raspbian" || $distrib_name == "neon" || $distrib_name == "elementary" ]]; then
    ...
    do_with_root apt-get install -y python3-pip python3-dev gcc lm-sensors wireless-tools
    ...
    do_with_root pacman -S python3-pip lm_sensors wireless_tools --noconfirm

elif [[ $distrib_name == "SuSE" ]]; then

    zypper --non-interactive in python3-pip python-devel gcc python-curses
    ...
    # Headers not needed for Arch, shipped with regular python packages
    do_with_root apk add py-pip python3-dev linux-headers musl-dev lm_sensors wireless-tools
